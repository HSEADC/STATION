import './tiser.css';
document.getElementById('button_1_main').addEventListener('mouseover', function () {
  document.getElementById('img_1_main').src = 'images/e46c8f875b48e7c71c49.svg';
  document.getElementsByTagName('body')[0].style = 'background-color: var(--bkl)';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('scroll', function () {
    ...
    var scrollPosition = window.scrollY; // Добавляем условие для начала анимации после 1000px скролла

    if (scrollPosition > 1600) {
      // Вычисляем новое положение для изображения
      var newLeftPosition = -1800 * ((scrollPosition - 1600) / window.innerHeight); // Применяем новое положение к изображению
...
document.addEventListener('scroll', function () {
  ...
  if (scrollOffset >= 2000) {
    var offsetAmount = Math.min(600, scrollOffset - 2000);
    var translateValue = "translateX(-".concat(offsetAmount, "px)");
